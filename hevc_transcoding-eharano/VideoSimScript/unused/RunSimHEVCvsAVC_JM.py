...
sim = RunSimJM(simName              = "HEVC_vs_AVC_JM17",
               encoderExe           = "../bin/JM/lencod.exe",
               decoderExe           = "../bin/JM/ldecod.exe",
               ...
               qpSet                = [[27],[28], [29], [30]],               # may be overriden by sequence specific configuration
               allowSeqOverride     = 1,
               cfgSeqDirOverride    = "cfg_jm/cfg_seq_HEVC_vs_AVC",
...
#               jobParamsLsfOverride = ['linux', 'max30min', True])           # only for LSF cluster: platform, queue, send results by email
               jobParamsLsfOverride = ['linux', 'sim_24h', True])            # only for LSF cluster: platform, queue, send results
...
sim.confMain['NumberBFrames'] = 3
sim.confMain['ExplicitHierarchyFormat'] = "B1r0B0e1B2e1"
sim.confMain['FrameRate'] = 50.0
