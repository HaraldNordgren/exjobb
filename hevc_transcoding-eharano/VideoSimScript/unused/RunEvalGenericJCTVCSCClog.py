...
### read config ###
assert len(sys.argv) == 6, "expected 5 arguments, the third argument indicates i:intra, l: low-delay, r:random access, the fourth mse psnr avail anchor 1 and 2 if shall be used otherwise 0, the fifth mse psnr avail test 1 and 2 if shall be used otherwise 0"
inputFile1 = sys.argv[1]
inputFile2 = sys.argv[2]
type = sys.argv[3]
msetypeAnchor = int(sys.argv[4])
msetypeTest = int(sys.argv[5])
           ...
           if okfind==0:
             ...
             if(msetypeAnchor==1):
               ...
               if(msetypeAnchor==2):
                 ...
                 offset=0
             bitrateEnc2 = float(alist[2+offset])
             psnrYEnc2 = float(alist[3+offset])
             psnrUEnc2 = float(alist[4+offset])
             psnrVEnc2 = float(alist[5+offset])
           else:
             aline = afile.readline()
          afile.close()
        else:
...
numdecimals=4
plotter.plotJCTVCPage(seqOK,seqs,results,results2,simIds1,simIds2, BDData, BDDataU, BDDataV,setavgBDY, setavgBDU, setavgBDV,setavgtime,type,numdecimals) #,lc)
