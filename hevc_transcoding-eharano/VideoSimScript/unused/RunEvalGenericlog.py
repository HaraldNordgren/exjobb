...
for seq in range(len(seqs)):
    ...
    for qpCnt in range(numQps):
        ...
        if(ok):
          ...
          results[seq][0].append(bitrateEnc2) #/1000)
        ...
        textConfig = jobsDir1 + '/' + tmp[0:ltmp-3] + '/config' + ".txt"
        conf = cfgFile.cfgFile(textConfig)
        removeDec = conf['removeDec']
        if ((removeDec==3) & (decode1)):
           # check decoder log file that MD5 was OK
           tmp=simIds1[seq][qpCnt]
           ltmp=len(tmp)
           textDec = jobsDir1 + '/' + tmp[0:ltmp-3] + '/log_dec_' + tmp[0:ltmp] + ".txt"
           ok = os.path.isfile(textDec)
           if(ok):
             logFile = open(textDec, 'r')
             ...
             logFile.close()
             if mismatch1:
                print "************************** Encoder/decoder mismatch!! %s **************************\n" % textDec

           else:
             print "No decoder log file found %s \n" % textDec
        else:
    ...
    for qpCnt in range(numQps):
        ...
        if(ok):
          ...
          results2[seq][0].append(bitrateEnc2) #/1000)
        ...
        #check if enc and dec match
        textConfig = jobsDir2 + '/' + tmp[0:ltmp-3] + '/config' + ".txt"
        conf = cfgFile.cfgFile(textConfig)
        removeDec = conf['removeDec']
        if ((removeDec==3) & (decode2)):
           # check decoder log file that MD5 was OK
           tmp=simIds2[seq][qpCnt]
           ltmp=len(tmp)
           textDec = jobsDir2 + '/' + tmp[0:ltmp-3] + '/log_dec_'+ tmp[0:ltmp] + ".txt"
           ok = os.path.isfile(textDec)
           if(ok):
             logFile = open(textDec, 'r')
             aline = logFile.readline()
