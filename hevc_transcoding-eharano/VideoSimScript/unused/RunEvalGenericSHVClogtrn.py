...
### read config ###
assert len(sys.argv) == 6, "expected four arguments, the third argument indicates i1,i2:intra 1.5x and 2x, p0,p1,p2: low-delay P snr,1.5x,2x, r0,r1,r2:random access snr, 1.5x, 2x b0,b1,b2: snr, 1.5x, 2x low-delay B, the fourth indicates delta qp (lowest is 0), the fifth indicates is anchor is(1 or 0)"
inputFile1 = sys.argv[1]
inputFile2 = sys.argv[2]
type = sys.argv[3]
dqpindex = int(sys.argv[4])
anchorScalable = int(sys.argv[5]) 
...
#decode1 = 0 # jsu to avoid decoding temporary
tmpSimIds2 = conf2['simIds']
decode2 = conf2['decode']
#decode2 = 0 # jsu to avoid decoding temporary
simIds1 = []
simIds2 = []
...
#for seq in range(1):
    ...
    for qpCnt in range(numQps):
        ...
        if(ok):
          ...
          while aline:
           #print aline             
           okfind = aline.rfind('written')
           #print "okfind=%d" % okfind
           if okfind>=0:
             if(anchorScalable):
               astr = str(aline)
               ...
               tmp = tmp[1:len(tmp)]
               #print tmp
               bitrateEnc2 = float(tmp) #float(alist[3])
               bitrateEnc2L0 = bitrateEnc2 #float(alist[3])
               psnrYEnc2L0 = psnrYEnc #float(alist[4])
               psnrUEnc2L0 = psnrUEnc #float(alist[5])
               psnrVEnc2L0 = psnrVEnc #float(alist[6])
               #aline = afile.readline() # extra line L1
             
             #astr = str(aline)
             #alist = astr.split(None)
             if(anchorScalable):
               bitrateEnc2 = bitrateEnc2 # + float(alist[3]) # base + enhance bitrate
               bitrateEnc2L1 = bitrateEnc2 #float(alist[3])
               psnrYEnc2 = psnrYEnc #float(alist[4])
               psnrUEnc2 = psnrUEnc #float(alist[5])
               psnrVEnc2 = psnrVEnc #float(alist[6])
             else:  
        ...
        if decode1:
            ...
            tmp = tmp[0:len(tmp)-1]
            tmp = tmp + '0'
            text = resultsSubDir1 + '/dec-rd_' + tmp + ".txt"
            print text
        else:
        ...
        if(ok):
          ...
          if(anchorScalable):
            results[seq][6].append(bitrateEnc2L0)
            results[seq][7].append(psnrYEnc2L0)
            results[seq][8].append(psnrUEnc2L0)
            results[seq][9].append(psnrVEnc2L0)
            results[seq][10].append(bitrateEnc2L1)          
          ...
          if(anchorScalable):
            resultslog[seq][6].append(bitrateEnc2L0)
            resultslog[seq][7].append(psnrYEnc2L0)
            resultslog[seq][8].append(psnrUEnc2L0)
            resultslog[seq][9].append(psnrVEnc2L0)
            resultslog[seq][10].append(bitrateEnc2L1)
          ...
          results[seq][7].append(0)
          results[seq][8].append(0)
          results[seq][9].append(0)
          results[seq][10].append(0)
          ...
          resultslog[seq][7].append(0)
          resultslog[seq][8].append(0)
          resultslog[seq][9].append(0)
          resultslog[seq][10].append(0)
    ...
    for qpCnt in range(numQps):
        ...
        if(ok):
          ...
          while aline:
           ...
           if okfind>=0:
             ...
             #astr = str(aline)
             #alist = astr.split(None)
             bitrateEnc2 = bitrateEnc2 #bitrateEnc2 + float(alist[3]) # base + enhance bitrate
             bitrateEnc2L1 = bitrateEnc2 #float(alist[3])
             psnrYEnc2 = psnrYEnc #float(alist[4])
             psnrUEnc2 = psnrUEnc #float(alist[5])
             psnrVEnc2 = psnrVEnc #float(alist[6])
        ...
        if decode2:
            ...
            tmp = tmp[0:len(tmp)-1]
            tmp = tmp + '0'
            text = resultsSubDir2 + '/dec-rd_' + tmp + ".txt"
        else:
        ...
        if(ok):
          ...
          results2[seq][6].append(bitrateEnc2L0)
          results2[seq][7].append(psnrYEnc2L0)
          results2[seq][8].append(psnrUEnc2L0)
          results2[seq][9].append(psnrVEnc2L0)          
          results2[seq][10].append(bitrateEnc2L1)
          ...
          resultslog2[seq][6].append(bitrateEnc2L0)
          resultslog2[seq][7].append(psnrYEnc2L0)
          resultslog2[seq][8].append(psnrUEnc2L0)
          resultslog2[seq][9].append(psnrVEnc2L0)
          resultslog2[seq][10].append(bitrateEnc2L1)
          if(anchorScalable==0):
            resultslog1[seq][0].append(1)
            resultslog1[seq][1].append(0)
            resultslog1[seq][2].append(0)
            resultslog1[seq][3].append(0)
            resultslog1[seq][4].append([0, 0, 0])
            resultslog1[seq][5].append([0, 0, 0])
            resultslog1[seq][6].append(0)
            resultslog1[seq][7].append(0)
            resultslog1[seq][8].append(0)
            resultslog1[seq][9].append(0)
            resultslog1[seq][10].append(0)
          ...
          results2[seq][7].append(0)
          results2[seq][8].append(0)
          results2[seq][9].append(0)
          results2[seq][10].append(0)
          ...
          resultslog2[seq][7].append(0)
          resultslog2[seq][8].append(0)
          resultslog2[seq][9].append(0)
          resultslog2[seq][10].append(0)
          theseqOK = 0
          text = text + 'NOT FOUND\n'
          print text
...
# test to include base layer rate to anchor bitrate when anchor is non-scalable 
if(anchorScalable==0):
  for seq in range(len(seqs)):
    if(seqOK[seq]):
     for i in range(len(resultslog[seq][0])):
       resultslog1[seq][0][i] =  resultslog[seq][0][i]+ resultslog2[seq][6][i]
      #print "tot bitrate[%d]=%f base bitrate=%f sing bitrate=%f\n" % (i,resultslog1[seq][0][i],resultslog2[seq][6][i], resultslog[seq][0][i]) 
...
for seq in range(len(seqs)):
  ...
  if(seqOK[seq]):    
    for i in range(len(resultslog[seq][0])):
            ...
            if(anchorScalable==0):
              resultslog1[seq][0][i] = math.log10(resultslog1[seq][0][i]) 
    avgenctime1 = [0, 0, 0]
    ...
    if(anchorScalable==0):
      d = BDelta.NDiff(resultslog1[seq][0], resultslog[seq][1], resultslog2[seq][0], resultslog2[seq][1]) 
      (average,avgOverlap) = d.bitrateDiff()     
      print "%s Gain over simulcast BDR %f\n" % (seqs[seq].name, average)
...
#outFileExcel = "%s/result_JCTVC_%s.xls" % (resultsDir2, outname)
#outFileExcel = "%s/results_SHVC_%s.xls" % (resultsDir2, outname)
...
numdecimals=4
plotter.plotJCTVCPage(seqOK,seqs,results,results2,simIds1,simIds2, BDData, BDDataU, BDDataV,setavgBDY, setavgBDU, setavgBDV,setavgtime,type,numdecimals)
# plot test sheet
plotter.plotSHVCPage(seqOK,seqs,results2,simIds2, BDData, BDDataU, BDDataV,setavgBDY, setavgBDU, setavgBDV,setavgtime,type,numdecimals,dqpindex,2)
#plot anchor sheet
if(anchorScalable):
  plotter.plotSHVCPage(seqOK,seqs,results,simIds1, BDData, BDDataU, BDDataV,setavgBDY, setavgBDU, setavgBDV,setavgtime,type,numdecimals,dqpindex,1) 
else:
  plotter.plotSHVCPage(seqOK,seqs,results,simIds1, BDData, BDDataU, BDDataV,setavgBDY, setavgBDU, setavgBDV,setavgtime,type,numdecimals,dqpindex,0)
...
for seq in range(len(seqs)):   
  if(seqOK[seq]):    
    ...
    print "%s: BDR Y %f U %f V %f" % (seqs[seq].name,BDData[seqnr][0],BDDataU[seqnr][0],BDDataV[seqnr][0])
    seqnr = seqnr + 1 
plotter.save()
