...
uint8
AllocateBitsOutputer(SessionData_t *Sessiondata_p,
                     BitsOutputer_t *BitsOutputer_p,
                     uint32 BufferLength)
{
  BitsOutputer_p->BufferStart_p = (uint8*)Sessiondata_p->Malloc_Function_p(BufferLength, D65_INTERNAL_DATA);
  if (BitsOutputer_p->BufferStart_p)
  {
    BitsOutputer_p->BufferPos    = 0;
...
void
AssignBitsOutputer(BitsOutputer_t *BitsOutputer_p,
                   uint8  *Buffer_p,
                   uint32  CurrBufferPos,
                   uint32  CurrByte,
                   uint32  CurrBytePos)
{
  BitsOutputer_p->BufferStart_p = Buffer_p;
  BitsOutputer_p->BufferPos     = CurrBufferPos;
  BitsOutputer_p->CurrByte      = CurrByte;
  BitsOutputer_p->CurrByte_Pos  = CurrBytePos;
}

void
ResetBitsOutputer(BitsOutputer_t *BitsOutputer_p)
{
    BitsOutputer_p->BufferPos    = 0;
    BitsOutputer_p->CurrByte     = 0;
    BitsOutputer_p->CurrByte_Pos = 0;
...
void
FreeBitsOutputer(SessionData_t *Sessiondata_p,
                 BitsOutputer_t *BitsOutputer_p)
{
  if (BitsOutputer_p->BufferStart_p)
  {
    Sessiondata_p->Free_Function_p(BitsOutputer_p->BufferStart_p, D65_INTERNAL_DATA);
    BitsOutputer_p->BufferStart_p = NULL;
  }
  BitsOutputer_p->BufferPos = 0;
  BitsOutputer_p->CurrByte  = 0;
  BitsOutputer_p->CurrByte_Pos = 0;
                ...
                const D65_DecoderParameters_t *Parameters_p)
{
  ...
  {
    Sessiondata_p->CurrentTiledata_p[i].TileStartCtuX  = 0;
    Sessiondata_p->CurrentTiledata_p[i].TileStartCtuY  = 0;
    Sessiondata_p->CurrentTiledata_p[i].TileStopCtuX   = 0;
    Sessiondata_p->CurrentTiledata_p[i].TileStopCtuY   = 0;
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
    Sessiondata_p->CurrentTiledata_p[i].TileStream     = NULL;
    Sessiondata_p->CurrentTiledata_p[i].TileStreamSize = 0;
#endif
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  Sessiondata_p->EntryNeedRewrite      = 0;
  Sessiondata_p->EntryOffsetLen        = 0;
  Sessiondata_p->BitOffsetBeforeEntry  = 0;
  Sessiondata_p->ByteValue             = 0;
  Sessiondata_p->BytePosBeforeEntry    = NULL;
#endif
...
#if GT_RESIDUAL_PRUNING ||GT_RESIDUAL_RECONSTRUCT
  Sessiondata_p->Write_Nal_Function_p = Parameters_p->Write_Nal_Function_p;
  Sessiondata_p->NalBufferSize        = Parameters_p->NalBufferSize;
  Sessiondata_p->NalOutputLocation    = Parameters_p->NalOutputLocation;
  Sessiondata_p->NalOutputBuffer      = NULL;
  Sessiondata_p->NalOutputBufferPos   = 0;

  Sessiondata_p->Parser.SubEncoder.BitsOutputer.BufferPos     = 0;
  Sessiondata_p->Parser.SubEncoder.BitsOutputer.BufferStart_p = NULL;
  Sessiondata_p->Parser.SubEncoder.BitsOutputer.CurrByte      = 0;
  Sessiondata_p->Parser.SubEncoder.BitsOutputer.CurrByte_Pos  = 0;
  Sessiondata_p->Parser.SubEncoder.OutputMode                 = 0;

  Sessiondata_p->NalOutputBuffer = (uint8*)Sessiondata_p->Malloc_Function_p(Sessiondata_p->NalBufferSize * sizeof(uint8), D65_INTERNAL_DATA);
  if (Sessiondata_p->NalOutputBuffer == NULL)
  {
    return D65_ERROR;
  }
#endif
  ...
  Sessiondata_p->InputYuvFile_p = Parameters_p->InputYuvFile_p;

  for (j = 0; j < MAX_CU_DEPTH; j++)
  {
    for (i = 0; i < MAX_SCAN_TYPE; i++)
    {
      Sessiondata_p->g_xScan[i][j] = NULL;
      Sessiondata_p->g_yScan[i][j] = NULL;
    }
  }

  Sessiondata_p->ResTrQuantArray   = NULL;
  Sessiondata_p->FrameNum          = 0;

  InitializeYUV(&(Sessiondata_p->SourceYUV));
...
D65_ReturnCode_t D65_Release(D65_Handle_t* Decoder_p)
{
  ...
  if (Sessiondata_p->NalOutputBuffer != NULL)
  {
    Sessiondata_p->Free_Function_p(Sessiondata_p->NalOutputBuffer, D65_INTERNAL_DATA);
    Sessiondata_p->NalOutputBuffer = NULL;
  }
#endif
...
AllocateTilesThreadMemory(SessionData_t *Sessiondata_p)
{
  ...
  for(i = 0; i < TilesNum; i++)
  {
    ...
    Sessiondata_p->Threaddata_p[i].ResTrQuantArray = (sint16*)Sessiondata_p->Malloc_Function_p(2*TransformSize*sizeof(sint16), D65_INTERNAL_DATA);
    if(Sessiondata_p->Threaddata_p[i].ResTrQuantArray == NULL)
    {
      return 1;
    }
    ...
    InitQuantGroupData(&Sessiondata_p->Threaddata_p[i].QgData);
...
void initTransformScanOrder(SessionData_t *Sessiondata_p, sint32 depth)
{
  const sint32 size      = 2 << depth;
  const sint32 numBlocks = (size < 4) ? 1 : (size >> 2);
  const sint32 max       = (size < 4) ? 2 : 4;
  ...
  //horizontal scan
  pos = 0;
  for (blockY = 0; blockY < numBlocks; blockY++)
  {
    for (blockX = 0; blockX < numBlocks; blockX++)
    {
      blockPos = blockY * 4 * size + blockX * 4;
      for (y = 0; y < max; y++)
      {
        for (x = 0; x < max; x++)
        {
          scanpos = y * size + x + blockPos;
          Sessiondata_p->g_yScan[SCAN_HORIZONTAL][depth][pos] = scanpos / size;
          Sessiondata_p->g_xScan[SCAN_HORIZONTAL][depth][pos] = scanpos % size;
          pos++;
        }
      }
    }
  ...
  for (blockX = 0; blockX < numBlocks; blockX++)
  {
    for (blockY = 0; blockY < numBlocks; blockY++)
    {
      ...
      for (x = 0; x < max; x++)
      {
        ...
        {
          scanpos = y * size + x + blockPos;
          Sessiondata_p->g_yScan[SCAN_VERTICAL][depth][pos] = scanpos / size;
          Sessiondata_p->g_xScan[SCAN_VERTICAL][depth][pos] = scanpos % size;
          pos++;
        }
      }
    }
  ...
  if (size < 8)
  {
    for (scanline = 0; pos < size*size; scanline++)
    {
      y = scanline;
      x = 0;
      ...
      while(y >= 0 && x < size)
      {
        //Sessiondata_p->g_transformScan[SCAN_DIAGONAL][depth][pos] = y * size + x ;
        Sessiondata_p->g_yScan[SCAN_DIAGONAL][depth][pos] = y;
        Sessiondata_p->g_xScan[SCAN_DIAGONAL][depth][pos] = x;
        x++;
        y--;
        pos++;
      }
  ...
  {
    blockWidth = 1 << (depth - 1);
    for (block = 0; block < blockWidth * blockWidth; block++)
    {
      pos = 0;
      if (size == 32)
      {
        blockOffset = transformLastScan32x32[block];
        Yoffset     = blockOffset / blockWidth;
        Xoffset     = blockOffset - Yoffset * blockWidth;
      }
      else
      {
        //blockOffset = Sessiondata_p->g_transformScan[SCAN_DIAGONAL][depth-2][block];
        Yoffset = Sessiondata_p->g_yScan[SCAN_DIAGONAL][depth - 2][block];
        Xoffset = Sessiondata_p->g_xScan[SCAN_DIAGONAL][depth - 2][block];
      }
      diagOffset = (Xoffset + Yoffset * size ) << 2;
      for (scanline=0; pos < 16; scanline++)
      {
        y = scanline;
        x = 0;
        while(y >= 4)
        ...
        while(y >= 0 && x < 4)
        {
          scanpos = y * size + x + diagOffset;
          //Sessiondata_p->g_transformScan[SCAN_DIAGONAL][depth][pos + (block<<4)] = y*size + x + diagOffset;
          Sessiondata_p->g_yScan[SCAN_DIAGONAL][depth][pos + (block << 4)] = scanpos / size;
          Sessiondata_p->g_xScan[SCAN_DIAGONAL][depth][pos + (block << 4)] = scanpos % size;
          pos++;
          x++;
          y--;
        }
...
AllocateSequenceMemory(SessionData_t *Sessiondata_p)
{
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  if (AllocateBitsOutputer(Sessiondata_p, &Sessiondata_p->Parser.SubEncoder.BitsOutputer, Sessiondata_p->NalBufferSize))
  {
    return 1;
  }
#endif
...
#if GT_RESIDUAL_RECONSTRUCT
  if(AllocateYUV(Sessiondata_p, &(Sessiondata_p->SourceYUV), Y_height, Y_width))
  {
    return 1;
  }
  ...
  for(j = 0; j < MAX_CU_DEPTH; j++)
  {
    ...
    for(i = 0; i < MAX_SCAN_TYPE; i++)
    {
      Sessiondata_p->g_xScan[i][j] = (sint32*)Sessiondata_p->Malloc_Function_p(cuSize * cuSize* sizeof(sint32), D65_INTERNAL_DATA);
      if(Sessiondata_p->g_xScan[i][j] == NULL)
      {
        return 1;
      }
      Sessiondata_p->g_yScan[i][j] = (sint32*)Sessiondata_p->Malloc_Function_p(cuSize * cuSize* sizeof(sint32), D65_INTERNAL_DATA);
      if(Sessiondata_p->g_yScan[i][j] == NULL)
      {
        return 1;
      }
    }
    initTransformScanOrder(Sessiondata_p, j);
  }

#if !(MULTI_THREADED)
  Sessiondata_p->ResTrQuantArray = (sint16*)Sessiondata_p->Malloc_Function_p(2 * MAX_TU_SIZE * MAX_TU_SIZE * sizeof(sint16), D65_INTERNAL_DATA);
  if(Sessiondata_p->ResTrQuantArray == NULL)
  {
    return 1;
  }
#endif
  ...
  for(i = 0; i < MAX_TILES_NUM; i++)
  {
    Sessiondata_p->CurrentTiledata_p[i].TileStream = (uint8*)Sessiondata_p->Malloc_Function_p(Sessiondata_p->NalBufferSize * sizeof(uint8), D65_INTERNAL_DATA);
    if (Sessiondata_p->CurrentTiledata_p[i].TileStream == NULL)
    {
      return 1;
    }
    Sessiondata_p->CurrentTiledata_p[i].TileStreamSize = 0;
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  ContextsBufferAssign(&Sessiondata_p->Parser.SubEncoder.CtxModels);
#endif
#endif
...
FreeSequenceMemory(SessionData_t *Sessiondata_p)
{
  ...
  for(i = 0; i < MAX_TILES_NUM; i++)
  {
    if (Sessiondata_p->CurrentTiledata_p[i].TileStream)
    {
      Sessiondata_p->Free_Function_p(Sessiondata_p->CurrentTiledata_p[i].TileStream, D65_INTERNAL_DATA);
    }
    Sessiondata_p->CurrentTiledata_p[i].TileStreamSize = 0;
  }
#endif
#else
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  FreeBitsOutputer(Sessiondata_p, &Sessiondata_p->Parser.SubEncoder.BitsOutputer);
#endif
#endif
  ...
  for (j = 0; j < MAX_CU_DEPTH; j++)
  {
    for (i = 0; i < MAX_SCAN_TYPE; i++)
    {
      if (Sessiondata_p->g_xScan[i][j])
      {
        Sessiondata_p->Free_Function_p(Sessiondata_p->g_xScan[i][j], D65_INTERNAL_DATA);
        Sessiondata_p->g_xScan[i][j] = NULL;
      }
      if (Sessiondata_p->g_yScan[i][j])
      {
        Sessiondata_p->Free_Function_p(Sessiondata_p->g_yScan[i][j], D65_INTERNAL_DATA);
        Sessiondata_p->g_yScan[i][j] = NULL;
      }
    }
  }

  if(Sessiondata_p->ResTrQuantArray)
  {
    Sessiondata_p->Free_Function_p(Sessiondata_p->ResTrQuantArray, D65_INTERNAL_DATA);
    Sessiondata_p->ResTrQuantArray = NULL;
  }

  FreeYuv(Sessiondata_p, &(Sessiondata_p->SourceYUV));
#endif
}
...
FreeTilesThreadMemory(SessionData_t *Sessiondata_p)
{
  ...
  if(Sessiondata_p->Threaddata_p)
  {
    for(i = 0; i < TilesNum; i++)
    {
      ...
      if(Sessiondata_p->Threaddata_p[i].ResTrQuantArray)
      {
        Sessiondata_p->Free_Function_p(Sessiondata_p->Threaddata_p[i].ResTrQuantArray, D65_INTERNAL_DATA);
        Sessiondata_p->Threaddata_p[i].ResTrQuantArray = NULL;
      }
