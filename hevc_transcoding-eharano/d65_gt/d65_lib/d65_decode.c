  ...
  *DecodedPicture_pp                 = NULL;
  ...
  if (ExceptionCode == 0)
  {
    ...
    else
    {
      ...
      //{
      //  BIT_ERROR(Sessiondata_p->LongJumpSlice, D65_MALLOC_ERROR, "Error when allocating buffer for outputting NAL");
      //}
#endif
...
#pragma message("TODO - Add support for more NAL unit types")
      ...
      case TSA_N:
        ...
        if (ReadYUV(&Sessiondata_p->SourceYUV, Sessiondata_p->InputYuvFile_p, Sessiondata_p->FrameNum++))
        {
          BIT_ERROR(Sessiondata_p->LongJumpSlice, D65_BIT_ERROR, "Failed to read source YUV file");
        }
#endif
        ...
        {
          if (Sessiondata_p->BytePosBeforeEntry == NULL)
          {
            BIT_ERROR(Sessiondata_p->LongJumpSlice, D65_BIT_ERROR, "Error when trying to output slice header");
          }

          PutSimpleCopy(Sessiondata_p->NalOutputBuffer,
                        InputNAL_p->NALData_p,
                        &Sessiondata_p->NalOutputBufferPos,
                        Sessiondata_p->BytePosBeforeEntry - InputNAL_p->NALData_p);
        }
        else
        {
          PutSimpleCopy(Sessiondata_p->NalOutputBuffer,
                        InputNAL_p->NALData_p,
                        &Sessiondata_p->NalOutputBufferPos,
                        Sessiondata_p->Parser.BitsReader.CurrentBuffer_p - InputNAL_p->NALData_p);
        }
#else
        PutSimpleCopy(Sessiondata_p->NalOutputBuffer,
                      InputNAL_p->NALData_p,
        ...
        ReturnValue = DecodeSliceData(Sessiondata_p, NALType, SliceStartCTUnr, Quant);
      ...
      case SUFFIX_SEI_NUT://SUFFIX_SEI_NUT
        ...
        DecodeSEI(Sessiondata_p, NALType); //...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
        PutSimpleCopy(Sessiondata_p->NalOutputBuffer,
                      InputNAL_p->NALData_p,
                      &Sessiondata_p->NalOutputBufferPos,
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  Sessiondata_p->Write_Nal_Function_p(Sessiondata_p->NalOutputLocation,
