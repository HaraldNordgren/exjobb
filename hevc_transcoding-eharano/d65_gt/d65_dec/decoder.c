...
DecodeSequence(const char *ProgramName,
               ...
               const char *BitstreamM_Filename,
#endif
#if GT_RESIDUAL_RECONSTRUCT
               const char * OriginalYuvFileNamePointer_p,
#endif
...
******************************************************************************/
void
WriteNalOnFile(void  *OuputLocation,
               uint8 *NalBuffer,
               uint32 NalLength)
{
  if(OuputLocation == NULL)
  {
    return;
  }

  fwrite(NalBuffer, sizeof(uint8), NalLength, (FILE*)OuputLocation);
}
#endif
...
#endif
#if GT_RESIDUAL_RECONSTRUCT
               const char *OriginalYuvFileName,
#endif
               const char *Output_Filename,
               const uint32 TraceFlag,
               const char *Trace_Filename
               ...
               )
{
...
#ifdef _WIN32
  {
...
#if GT_RESIDUAL_PRUNING ||GT_RESIDUAL_RECONSTRUCT
    NalOutFile_p = NULL;
    if(BitstreamM_Filename != NULL)
    {
      NalOutFile_p = fopen(BitstreamM_Filename, "wb");
      if (NalOutFile_p == NULL)
      {
        Usage(ProgramName, "Could not open output file");
        return(1);
      }
...
#if GT_RESIDUAL_RECONSTRUCT
    InputYuvFile_p = NULL;
    if(OriginalYuvFileName != NULL)
    {
      InputYuvFile_p = fopen(OriginalYuvFileName,"rb");
      if(InputYuvFile_p == NULL)
      {
        Usage(ProgramName,"Could not open input yuv file");
        return(1);
      }
    }
#endif
    ...
    BitstreamBuffer = (uint8*)malloc(FileSize);
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
    Parameters_p.Write_Nal_Function_p = WriteNalOnFile;
    Parameters_p.NalBufferSize        = 1000000;
    Parameters_p.NalOutputLocation    = NalOutFile_p;
#endif

#if GT_RESIDUAL_RECONSTRUCT
    Parameters_p.InputYuvFile_p = InputYuvFile_p;
#endif
     ...
     const char *Argv_pp[])
{
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  uint8 *NalFileNamePointer_p = NULL;
#endif
#if GT_RESIDUAL_RECONSTRUCT
  uint8 *OriginalYuvFileNamePointer_p; 
#endif
...
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
  uint8 Nal_Filename[256]       = "\0";
#endif

#if GT_RESIDUAL_RECONSTRUCT
  uint8 Original_YUV_Filename[256] = "\0";
#endif
  ...
  NalFileNamePointer_p    = NULL;
#endif

#if GT_RESIDUAL_RECONSTRUCT
  OriginalYuvFileNamePointer_p = NULL;
#endif
  /* Parse parameters */
  for (i = 1; i < Argc; i++) 
  {
    if (*(Argv_pp[i]) == '-') 
    {
      ...
      case 'n':
        if(i < Argc-1)
        {
          strcpy(Nal_Filename, Argv_pp[++i]);
          NalFileNamePointer_p = Nal_Filename;
        }
        break;
#endif
    ...
    case 'u':// yuv file
      if(i < Argc-1)
      {
        strcpy(Original_YUV_Filename, Argv_pp[++i]);
        OriginalYuvFileNamePointer_p = Original_YUV_Filename;
      }
      break;
#endif
  ...
  DecodeSequence(Argv_pp[0],
                 Bitstream_Filename,
#if GT_RESIDUAL_PRUNING || GT_RESIDUAL_RECONSTRUCT
                 NalFileNamePointer_p,
#endif
#if GT_RESIDUAL_RECONSTRUCT
                 OriginalYuvFileNamePointer_p,
#endif
