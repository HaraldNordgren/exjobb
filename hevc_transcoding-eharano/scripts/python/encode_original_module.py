...
def encode_original(original_file, cfg_file, QPs):
    ...
    output_file_cfg_mode_added = "%s_%s" % (output_file_framerate_replaced, cfg_mode)

    output_folder = "%s/%s" % (directories.bitstream_folder, output_file_cfg_mode_added)
    ...
    for qp in QPs:
        ...
        output_file_shortpath = "%s_qp-%d" % (output_file_cfg_mode_added, qp)
