...
import downscaling, filenames, paths, raw_video, time_string, command_line, transcoding_scripts
...
def transcode():
    ...
    paths.create_if_needed(directories.storage_folder)
    paths.create_if_needed(directories.tmp_folder)
    paths.create_if_needed(directories.scripts_folder)
    ...
    for original_file in originals:
        ...
        hq_bitstream_generic = "%s_%df_%s" % (hq_bitstream_framerate_replaced, config.frames, cfg_mode)

        sequence_folder_modular = hq_bitstream_generic
        #sequence_folder = "%s/%s" % (transcoding_folder, sequence_folder_modular)
        ...
        downscaled_originals_folder = "%s/%s" % (transcoding_folder, downscaled_originals_folder_modular)
        ...
        for downscale_parameters in downscale_parameter_list:
            ...
            downscaled_original = "%s/%s" % (transcoding_folder, downscaled_original_modular)
        ...
        for qp_hq in QP_hq:
            ...
            hq_bitstream_shortpath = "%s_%s" % (hq_bitstream_generic, qp_hq_string)

            hq_bitstream_folder_modular = "%s/%s" % (sequence_folder_modular, qp_hq_string)
            
            ## Only for creating folder ##
            hq_bitstream_folder = "%s/%s" % (transcoding_folder, sequence_folder_modular)            
            ...
            hq_bitstream = "%s/%s" % (transcoding_folder, hq_bitstream_modular)
            ...
            downscaled_originals_encoded_modular = []

            downscaled_originals_encoded_folder_modular = "%s/downscaled_originals_encoded" % hq_bitstream_folder_modular
            
            ## Only for creating folder ##
            downscaled_originals_encoded_folder = "%s/%s" % (transcoding_folder, downscaled_originals_encoded_folder_modular)
            ...
            for i in range(len(downscaled_originals)):
                ...
                downscaled_original_encoded = "%s/%s" % (transcoding_folder, downscaled_originals_encoded_folder_modular)
                ...
                downscaled_originals_encoded_modular.append(downscaled_original_encoded_modular)
            ...
            hq_bitstream_decoded = "%s/%s" % (transcoding_folder, hq_bitstream_decoded_modular)
            ...
            for i in range(len(downscale_parameter_list)):
                ...
                downscale_folder_modular = "%s/%dp" % (hq_bitstream_folder, downscaled_height)

                ## Only for creating folder ##
                downscale_folder = "%s/%s" % (transcoding_folder, downscale_folder_modular)
                ...
                downscaled_file = "%s/%s" % (transcoding_folder, downscaled_file_modular)
                ...
                hq_bitstream_decoded_dec_order_downscaled = "%s/%s" % (transcoding_folder, hq_bitstream_decoded_dec_order_downscaled_modular)
                ...
                for qp_lq in QP_lq:
                    ...
                    ## Only for creating folder ##
                    lq_bitstream_folder = "%s/%s" % (transcoding_folder, lq_bitstream_folder_modular)
                    ...
                    rdoq_0_file = "%s/%s" % (transcoding_folder, rdoq_0_file_modular)
                    ...
                    pruned_file = "%s/%s" % (transcoding_folder, pruned_file_modular)
                    ...
                    if debug.debug_1:
                        ...
                        pruned_file_decoded = "%s/%s" % (transcoding_folder, pruned_file_decoded_modular)
                    ...
                    reconstructed_file = "%s/%s" % (transcoding_folder, reconstructed_file_modular)
                    ...
                    reconstructed_file_decoded = "%s/%s" % (transcoding_folder, reconstructed_file_decoded_modular)
                    ...
                    script_name_shortpath = "transcode_%s_%s_%sp_%s.py" % (hq_bitstream_generic, qp_hq_string, downscaled_height, qp_lq_string)
                    script_name = "%s/%s" (directories.scripts_folder, script_name_shortpath)
