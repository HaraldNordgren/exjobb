...
import downscaling, filenames, paths, raw_video, definitions, binaries
...
sequence_folder = "%s/%s" % (definitions.output_folder, hq_bitstream_shortpath)
...
for downscale_parameters in downscale_parameter_list:
    ...
    rdoq_0_cmd = "%s -c %s -i %s -b %s -fr %d -f %d -wdt %d -hgt %d --RDOQ=0 -SBH 0 --RDOQTS=0" % (binaries.hm_encoder, 
        definitions.cfg_file, downscaled_file, rdoq_0_file, definitions.framerate, definitions.frames, downscaled_width, downscaled_height)
    ...
    #raw_video.play_yuv(reconstructed_file_decoded, downscaled_width, downscaled_height)
